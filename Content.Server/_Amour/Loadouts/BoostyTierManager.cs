...
public sealed class BoostyTierManager : IBoostyTierManager
{
    ...
    private void EnsureRefresh(Guid userId)
    {
        ...
        {
            _sawmill?.Debug($"EnsureRefresh: already in-flight for {userId}");
            return;
        }

        _sawmill?.Debug($"EnsureRefresh: starting refresh task for {userId}");
        var task = RefreshAsync(userId);
        if (!_inflight.TryAdd(userId, task))
        {
            _sawmill?.Debug($"EnsureRefresh: failed to add to inflight for {userId}");
            return;
        }
        ...
        _ = task.ContinueWith(t =>
        {
            ...
            if (t.IsFaulted)
                ...
                _sawmill?.Debug($"EnsureRefresh: completed for {userId}");
        });
    }
